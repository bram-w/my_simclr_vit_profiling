...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, shallow_prompt=None):
        x = self.patch_embed(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        # BW: dist token is distill token from Deit I think
        if self.dist_token is None:
        ...
        # BW: Format is BS x seq_length x dim
        # anticipate shallow_prompt being num_prompt_tokens x dim
        # doesn't have positional embedding added  
        # in spirit of keeping prompt module separate from the .visual model this make sense
        # Also, shouldn't matter b/c perceiver/prompter will have positional embeddings
        # on input so tokens will be distinct anyways, this would just serve as a bias term
        if shallow_prompt is not None:
            x = torch.cat([x, shallow_prompt.unsqueeze(0).expand(x.shape[0], -1, -1)], dim=1)
    ...
    def forward(self, x, shallow_prompt=None):
        x = self.forward_features(x, shallow_prompt=shallow_prompt)
